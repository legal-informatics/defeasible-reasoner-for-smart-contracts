...
contract TermBuilder {
	...
	function pred(bytes memory _name, Term memory _term1, Term memory _term2, Term memory _term3, Term memory _term4) internal pure returns (Term memory) {
		Term memory p = pred(_name, 4);
		p.arguments[0] = _term1;
		p.arguments[1] = _term2;
		p.arguments[2] = _term3;
		p.arguments[3] = _term4;
		return p;
	}

	function pred(bytes memory _name, Term memory _term1, Term memory _term2, Term memory _term3, Term memory _term4, Term memory _term5) internal pure returns (Term memory) {
		Term memory p = pred(_name, 5);
		p.arguments[0] = _term1;
		p.arguments[1] = _term2;
		p.arguments[2] = _term3;
		p.arguments[3] = _term4;
		p.arguments[4] = _term5;
		return p;
	}

	function predNot(bytes memory _name, Term memory _term1) internal pure returns (Term memory) {
		Term memory p = pred(_name, 1);
		p.kind = TermKind.PredicateNot;
		p.arguments[0] = _term1;
		return p;
	}
	function predNot(bytes memory _name, Term memory _term1, Term memory _term2) internal pure returns (Term memory) {
		Term memory p = pred(_name, 2);
		p.kind = TermKind.PredicateNot;
		p.arguments[0] = _term1;
		p.arguments[1] = _term2;
	...
	function predNot(bytes memory _name, Term memory _term1, Term memory _term2, Term memory _term3) internal pure returns (Term memory) {
		Term memory p = pred(_name, 3);
		p.kind = TermKind.PredicateNot;
		p.arguments[0] = _term1;
		p.arguments[1] = _term2;
	...
	function predNot(bytes memory _name, Term memory _term1, Term memory _term2, Term memory _term3, Term memory _term4) internal pure returns (Term memory) {
		Term memory p = pred(_name, 4);
		p.kind = TermKind.PredicateNot;
		p.arguments[0] = _term1;
		p.arguments[1] = _term2;
...
library RuleBuilder {
	...
	function add(Rule[] storage _rules, Term memory _head, Term memory _body1, Term memory _body2, Term memory _body3, Term memory _body4) internal {
		add(_rules, _head);
		_rules[_rules.length - 1].body.push().fromMemory(_body1);
		_rules[_rules.length - 1].body.push().fromMemory(_body2);
		_rules[_rules.length - 1].body.push().fromMemory(_body3);
		_rules[_rules.length - 1].body.push().fromMemory(_body4);
	}
	function add(Rule[] storage _rules, Term memory _head, Term memory _body1, Term memory _body2, Term memory _body3, Term memory _body4, Term memory _body5) internal {
		add(_rules, _head);
		_rules[_rules.length - 1].body.push().fromMemory(_body1);
		_rules[_rules.length - 1].body.push().fromMemory(_body2);
		_rules[_rules.length - 1].body.push().fromMemory(_body3);
		_rules[_rules.length - 1].body.push().fromMemory(_body4);
		_rules[_rules.length - 1].body.push().fromMemory(_body5);
	}
