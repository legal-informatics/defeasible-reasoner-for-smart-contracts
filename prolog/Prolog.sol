...
library Prolog {
	...
	function query(Term memory _goal, Rule[] memory data, Rule[] storage _rules, Substitution.Info storage _substitutions) internal returns (bool) {
		Term[] memory goals = new Term[](1);
		goals[0] = _goal;

		return query(goals, data, _rules, _substitutions);
	}

	function query(Term[] memory _goals, Rule[] memory data, Rule[] storage _rules, Substitution.Info storage _substitutions) internal returns (bool) {
		if (_goals.length == 0)
			return true;
		...
		if (_rules.length == 0)
			return false;
// ==========================================
		if (_goals[0].kind == TermKind.PredicateNot) {
			for (uint i = 0; i < data.length + _rules.length; ++i) {
				_substitutions.dup();
				Rule memory rewrittenRule = rewriteVariables(    ((i<data.length) ? data[i] : _rules[i-data.length])   , _substitutions.usedKeys.length);
				if (Unification.unify(_goals[0], rewrittenRule.head, _substitutions)) {
					Term[] memory unifiedGoals = new Term[](rewrittenRule.body.length);
					for (uint g = 0; g < rewrittenRule.body.length; ++g) {
						unifiedGoals[g] =  rewrittenRule.body[g];
					}

					bool success = query(unifiedGoals, data, _rules, _substitutions);
			...
			Term[] memory otherUnifiedGoals = new Term[](_goals.length-1);
			for (uint g = 1; g < _goals.length; ++g) {
				otherUnifiedGoals[g - 1] =  _goals[g];
			}
			bool success = query(otherUnifiedGoals, data, _rules, _substitutions);
			return success;
		}
// ==========================================
